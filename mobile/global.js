global.api = "192.168.1.155";
global.userId = null;
global.userName = null;

global.dictionary = {
    FAVORITES: { PT: "Favoritos", EN: "Favorites" },
    AGENDA: { PT: "Agenda", EN: "Agenda" },
    SEARCH: { PT: "Pesquisa", EN: "Search" },
    EVENTS: { PT: "Eventos", EN: "Events" },
    CATEGORY: { PT: "Categoria", EN: "Category" },
    CATEGORIES: { PT: "Categorias", EN: "Categories" },
    ENTITY: { PT: "Entidade", EN: "Entity" },
    ENTITIES: { PT: "Entidades", EN: "Entities" },
    ALL: { PT: "Todos", EN: "All" },
    NO_EVENTS: { PT: "Não há eventos futuros para mostrar.", EN: "There are no future events to display." },
    AT: { PT: "em", EN: "at" },
    PRICE: { PT: "Preço", EN: "Price" },
    FREE: { PT: "Gratuito", EN: "Free" },
    MAP: { PT: "Mapa", EN: "Map" },
    CALENDAR: { PT: "Calendário", EN: "Calendar" },
    APP_NAME: { PT: "Agenda Cultural da Universidade do Porto", EN: "Porto University's Cultural Agenda" },
    GOOGLE_SIGNIN: { PT: "Entrar com conta Google", EN: "Sign in with Google account" },
    NO_SEARCH_RESULTS: { PT: "Não há eventos para a pesquisa feita.", EN: "There are no events that match your search." },
    NO_FAVORITES: { PT: "Não há eventos favoritos para mostrar.", EN: "There are no favorite events." }
};

global.calendar = {
    1: { PT: 'JAN', EN: 'JAN' },
    2: { PT: 'FEV', EN: 'FEB' },
    3: { PT: 'MAR', EN: 'MAR' },
    4: { PT: 'ABR', EN: 'APR' },
    5: { PT: 'MAI', EN: 'MAY' },
    6: { PT: 'JUN', EN: 'JUN' },
    7: { PT: 'JUL', EN: 'JUL' },
    8: { PT: 'AGO', EN: 'AUG' },
    9: { PT: 'SET', EN: 'SEP' },
    10: { PT: 'OUT', EN: 'OCT' },
    11: { PT: 'NOV', EN: 'NOV' },
    12: { PT: 'DEZ', EN: 'DEC' }
};

global.fullCalendar = {
    1: { PT: 'Janeiro', EN: 'January' },
    2: { PT: 'Fevereiro', EN: 'February' },
    3: { PT: 'Março', EN: 'March' },
    4: { PT: 'Abril', EN: 'April' },
    5: { PT: 'Maio', EN: 'May' },
    6: { PT: 'Junho', EN: 'June' },
    7: { PT: 'Julho', EN: 'July' },
    8: { PT: 'Agosto', EN: 'August' },
    9: { PT: 'Setembro', EN: 'September' },
    10: { PT: 'Outubro', EN: 'October' },
    11: { PT: 'Novembro', EN: 'November' },
    12: { PT: 'Dezembro', EN: 'December' }
};
